...
const AboutUs = () => {
  ...
  return (
    <div className="bg-off-white dark:bg-gray-900 min-h-screen pt-0 transition-colors duration-300">
      {/* Hero Section with animated gradient background */}
      <section ref={heroRef} className={`relative overflow-hidden bg-warm-light-blue dark:bg-gray-800 py-20 px-4 md:px-0 flex flex-col items-center text-center transition-all duration-1000 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="relative z-10 flex flex-col items-center w-full">
          <img src="/logo512.png" alt="House of Humanity Logo" className="h-28 w-auto mx-auto mb-8 mt-6 drop-shadow-lg bg-white dark:bg-gray-700 rounded-full p-2 border-4 border-warm-light-blue dark:border-gray-600" style={{maxWidth:'120px'}} />
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center mt-6">
            <div className="bg-white/80 dark:bg-gray-700/80 rounded-xl shadow-lg px-8 py-6 max-w-xs mx-auto border-t-4 border-primary-blue transition-transform duration-500 ease-out animate-fade-in-card hover:scale-105 hover:-translate-y-1 cursor-pointer">
              <h2 className="text-xl font-bold text-primary-blue mb-2">Our Mission</h2>
              <p className="text-dark-gray dark:text-gray-200 text-base">To touch lives, offer hope, and work tirelessly for a world where compassion leads the way.</p>
            </div>
            <div className="bg-white/80 dark:bg-gray-700/80 rounded-xl shadow-lg px-8 py-6 max-w-xs mx-auto border-t-4 border-fresh-green transition-transform duration-500 ease-out animate-fade-in-card delay-200 hover:scale-105 hover:-translate-y-1 cursor-pointer">
              <h2 className="text-xl font-bold text-fresh-green mb-2">Our Vision</h2>
              <p className="text-dark-gray dark:text-gray-200 text-base">A future where no one is left behind, and every person, family, and community can realize their potential.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section ref={quoteRef} className={`py-8 px-4 md:px-0 flex justify-center transition-all duration-1000 ${quoteVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="bg-white/90 dark:bg-gray-800/90 rounded-2xl shadow-lg px-8 py-8 max-w-2xl mx-auto border-l-4 border-primary-blue flex flex-col items-center animate-fade-in-card delay-400 transition-transform duration-500 hover:scale-105 hover:-translate-y-1 cursor-pointer">
          <Quote className="w-8 h-8 text-primary-blue mb-2 animate-bounce-slow" />
          <p className="text-xl italic text-dark-gray dark:text-gray-200 mb-2">"If not us, then who?"</p>
      ...
      <section ref={aboutRef} className={`py-16 px-4 md:px-0 max-w-5xl mx-auto transition-all duration-1000 ${aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <p className="max-w-3xl mx-auto text-lg text-dark-gray dark:text-gray-200 mb-10 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-8">
          {focusAreas.map((area, idx) => (
            <div key={area.title} className="bg-warm-light-blue dark:bg-gray-700 rounded-2xl shadow-md p-6 flex flex-col items-center text-center hover:scale-105 hover:shadow-xl transition-transform duration-300 group cursor-pointer">
              ...
              <p className="text-dark-gray dark:text-gray-200 text-sm group-hover:text-primary-blue transition-colors duration-300">{area.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* History Section */}
      <section ref={historyRef} className={`py-16 px-4 md:px-0 bg-white dark:bg-gray-800 border-t border-warm-light-blue dark:border-gray-600 transition-all duration-1000 ${historyVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1">
            <h2 className="text-3xl md:text-4xl font-bold text-primary-blue mb-6 header-font">Our Story</h2>
            <p className="text-dark-gray dark:text-gray-200 text-lg mb-6">
            ...
            <p className="text-dark-gray dark:text-gray-200 text-base mb-4">
            ...
            <p className="text-dark-gray dark:text-gray-200 text-base">
          ...
          <div className="flex-1 flex flex-col items-center">
            <img src="/public/images/we-fight-for-humanity.png" alt="Volunteers helping community" className="rounded-2xl shadow-lg w-full max-w-xs mb-6 animate-fade-in" />
            <img src="/public/Gallery/image4.webp" alt="House of Humanity team" className="rounded-2xl shadow-lg w-full max-w-xs animate-fade-in delay-200" />
          </div>
        </div>
      </section>
      {/* Custom Animations */}
