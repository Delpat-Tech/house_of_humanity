...
const baseVolunteers: Volunteer[] = [
  { name: 'Aarav', img: '/images/volunteer1.jpg', intro: 'Loves teaching kids.' },
  { name: 'Priya', img: '/images/volunteer2.jpg', intro: 'Community health advocate.' },
  { name: 'Rohan', img: '/images/volunteer3.jpg', intro: 'Sports and youth mentor.' },
  { name: 'Simran', img: '/images/volunteer4.jpg', intro: 'Nutrition and wellness lead.' },
];
  ...
  return (
    <div className="bg-off-white min-h-screen pt-0">
      ...
      <section className="max-w-5xl mx-auto py-16 px-4 md:px-0 pt-32">
        <h2 className="text-3xl md:text-4xl font-bold text-primary-blue mb-10 header-font text-center">Founders Desk</h2>
            ...
            return (
              <div
                ...
                className={`flex-1 bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center text-center border-t-4 border-fresh-green transition-shadow duration-300 transition-all duration-1000 animate-float-card
                  hover:scale-105 hover:shadow-2xl hover:border-primary-blue ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              >
                <img src={f.img} alt={f.name} className="w-40 h-40 object-cover rounded-full border-4 border-primary-blue mb-6 shadow-md" />
                <h3 className="text-xl font-bold text-primary-blue mb-1">{f.name}</h3>
                <span className="text-fresh-green font-semibold mb-4">{f.title}</span>
      ...
      <section className="bg-warm-light-blue py-16 px-4 md:px-0">
        <h2 className="text-3xl md:text-4xl font-bold text-primary-blue mb-4 header-font text-center">Our Team</h2>
        <p className="text-dark-gray text-center mb-8 max-w-2xl mx-auto">Meet our passionate volunteers! Here's a glimpse of our team—each card represents a story of compassion and impact. Click below to see everyone!</p>
        <PolaroidCollage volunteers={collageVolunteers} />
        <div className="flex justify-center mt-8">
          <button onClick={() => setShowModal(true)} className="btn-primary shadow-lg hover:scale-105 transition-transform">See All Volunteers</button>
        </div>
        {showModal && <AllVolunteersModal volunteers={allVolunteers} onClose={() => setShowModal(false)} />}
      </section>
      ...
      {volunteers.map((v, idx) => {
        ...
        return (
          <FlipCard
            key={idx}
            volunteer={v}
            style={{ transform: `rotate(${rotate}deg)`, zIndex: z, position: 'relative' }}
            refProp={ref}
            visible={isVisible}
          />
        );
...
function FlipCard({ volunteer, style, refProp, visible }: FlipCardProps) {
  ...
  return (
    <div
      ref={refProp}
      className={`perspective w-32 h-40 cursor-pointer transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
      style={style}
    ...
    >
      <div className={`relative w-full h-full transition-transform duration-700 transform-style-preserve-3d ${flipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute inset-0 bg-white polaroid-card rounded-xl flex flex-col items-center justify-center p-2 backface-hidden">
          <img src={volunteer.img} alt={volunteer.name} className="w-16 h-16 object-cover rounded-full border-2 border-primary-blue mb-2" />
          <span className="text-primary-blue font-semibold text-xs text-center">{volunteer.name}</span>
        </div>
        {/* Back */}
        <div className="absolute inset-0 bg-fresh-green polaroid-card rounded-xl flex flex-col items-center justify-center p-2 rotate-y-180 backface-hidden">
          <span className="text-dark-gray font-semibold text-xs mb-1 text-center">{volunteer.name}</span>
          <span className="text-dark-gray text-xs text-center">{volunteer.intro}</span>
        </div>
      </div>
    </div>
  );
...
function AllVolunteersModal({ volunteers, onClose }: { volunteers: Volunteer[]; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <div className="bg-off-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto p-0 relative">
        ...
        <div className="sticky top-0 z-20 bg-off-white flex justify-end items-center px-8 pt-6 pb-2 rounded-t-2xl">
          <button onClick={onClose} className="bg-white rounded-full shadow text-primary-blue font-bold text-lg w-10 h-10 flex items-center justify-center border border-primary-blue hover:bg-primary-blue hover:text-white transition-colors">&times;</button>
        </div>
        <h3 className="text-2xl font-bold text-primary-blue mb-6 text-center">All Volunteers</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 px-8 pb-8">
          {volunteers.map((v, idx) => {
            const { ref, isVisible } = useScrollFadeIn();
            return <FlipCard key={idx} volunteer={v} refProp={ref} visible={isVisible} />;
          })}
        </div>
      </div>
    </div>
